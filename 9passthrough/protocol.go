...
type MmapRegisterRequest struct {
	FilePath string
	Length   uint64
	Flags    uint32 // MAP_SHARED, MAP_PRIVATE, etc.
}
...
// MmapUnregisterRequest - Client is done with mmap
...
// Encoding functions (simple binary encoding)

// EncodeMmapRegisterRequest encodes a register request
...
// DecodeMmapRegisterRequest decodes a register request
func DecodeMmapRegisterRequest(data []byte) (*MmapRegisterRequest, error) {
	if len(data) < 4 {
		return nil, fmt.Errorf("data too short")
	}

	pathLen := binary.LittleEndian.Uint32(data[0:4])
	if len(data) < int(4+pathLen+8+4) {
		return nil, fmt.Errorf("data too short for path")
	}

	req := &MmapRegisterRequest{
		FilePath: string(data[4 : 4+pathLen]),
		Length:   binary.LittleEndian.Uint64(data[4+pathLen : 4+pathLen+8]),
		Flags:    binary.LittleEndian.Uint32(data[4+pathLen+8:]),
	}

	return req, nil
}

// EncodeMmapRegisterResponse encodes a register response
func EncodeMmapRegisterResponse(resp *MmapRegisterResponse) []byte {
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, resp.MmapID)
	return buf
}

// DecodeMmapRegisterResponse decodes a register response
...
// EncodeMmapBulkReadRequest encodes a bulk read request
...
// DecodeMmapBulkReadRequest decodes a bulk read request
func DecodeMmapBulkReadRequest(data []byte) (*MmapBulkReadRequest, error) {
	if len(data) < 24 {
		return nil, fmt.Errorf("data too short")
	}

	return &MmapBulkReadRequest{
		MmapID: binary.LittleEndian.Uint64(data[0:8]),
...
// EncodeMmapBulkReadResponse encodes a bulk read response
func EncodeMmapBulkReadResponse(resp *MmapBulkReadResponse) []byte {
...
// DecodeMmapBulkReadResponse decodes a bulk read response
...
// EncodeMmapWriteRequest encodes a write request
...
// DecodeMmapWriteRequest decodes a write request
func DecodeMmapWriteRequest(data []byte) (*MmapWriteRequest, error) {
	if len(data) < 20 {
		return nil, fmt.Errorf("data too short")
	}
	...
	return &MmapWriteRequest{
		MmapID: binary.LittleEndian.Uint64(data[0:8]),
		Offset: binary.LittleEndian.Uint64(data[8:16]),
		Data:   data[20 : 20+dataLen],
	}, nil
}

// EncodeMmapWriteResponse encodes a write response
func EncodeMmapWriteResponse(resp *MmapWriteResponse) []byte {
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, resp.BytesWritten)
	return buf
}

// DecodeMmapWriteResponse decodes a write response
...
// EncodeMmapNotifyMessage encodes a notify message
func EncodeMmapNotifyMessage(msg *MmapNotifyMessage) []byte {
	buf := make([]byte, 24)
	binary.LittleEndian.PutUint64(buf[0:8], msg.MmapID)
	binary.LittleEndian.PutUint64(buf[8:16], msg.Offset)
	binary.LittleEndian.PutUint64(buf[16:24], msg.Length)
	return buf
}

// DecodeMmapNotifyMessage decodes a notify message
...
// EncodeMmapUnregisterRequest encodes an unregister request
func EncodeMmapUnregisterRequest(req *MmapUnregisterRequest) []byte {
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, req.MmapID)
	return buf
}

// DecodeMmapUnregisterRequest decodes an unregister request
func DecodeMmapUnregisterRequest(data []byte) (*MmapUnregisterRequest, error) {
	if len(data) < 8 {
		return nil, fmt.Errorf("data too short")
	}

	return &MmapUnregisterRequest{
		MmapID: binary.LittleEndian.Uint64(data[0:8]),
	}, nil
}

// EncodeErrorResponse encodes an error response
func EncodeErrorResponse(err *ErrorResponse) []byte {
	msgLen := uint32(len(err.Message))
	buf := make([]byte, 8+msgLen)
	binary.LittleEndian.PutUint32(buf[0:4], err.Code)
	binary.LittleEndian.PutUint32(buf[4:8], msgLen)
	copy(buf[8:], []byte(err.Message))
	return buf
}

// DecodeErrorResponse decodes an error response
